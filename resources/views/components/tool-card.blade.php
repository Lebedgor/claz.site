...
<a href="{{ route('tools.show', $tool, false) }}"
   class="group flex flex-col gap-3 rounded-2xl bg-white p-5 ring-1 ring-zinc-200/80 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-indigo-500/10 hover:ring-indigo-200 dark:bg-zinc-900 dark:ring-zinc-800 dark:hover:ring-indigo-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-500">
    <div class="flex items-center gap-3">
        @if ($tool->logo_url !== null)
            <img src="{{ $tool->logo_url }}" alt="{{ $name }}" loading="lazy" decoding="async"
                 class="h-11 w-11 shrink-0 rounded-xl object-cover ring-1 ring-zinc-200 dark:ring-zinc-700">
        @else
        ...
        @endif
        <div class="min-w-0">
            <h3 class="truncate font-semibold text-zinc-900 transition group-hover:text-indigo-600 dark:text-zinc-100 dark:group-hover:text-indigo-400">{{ $name }}</h3>
            @if (filled($tool->vendor))
                <p class="truncate text-xs text-zinc-500 dark:text-zinc-400">{{ $tool->vendor }}</p>
            @endif
        </div>
        @if ($tool->rating_avg !== null)
            <span class="ml-auto inline-flex shrink-0 items-center gap-1 rounded-lg bg-linear-to-br from-amber-50 to-amber-100 px-2 py-1 text-sm font-bold text-amber-700 ring-1 ring-amber-200 dark:from-amber-950 dark:to-amber-900 dark:text-amber-400 dark:ring-amber-800">
    ...
    <div class="mt-auto flex items-center justify-between">
        ...
        <span aria-hidden="true" class="-translate-x-1 text-indigo-600 opacity-0 transition duration-300 group-hover:translate-x-0 group-hover:opacity-100 dark:text-indigo-400">
