...
7. Content import: **no longer manual** — the local database is the source of truth for content (articles, tools, comments, settings): every deploy transfers the FULL local DB and storage files to production (see step 8). The uploads volume shadows the image copy, but git-tracked article images are also auto-seeded into the volume on every container start: the Dockerfile copies `storage/app/public` to `/opt/uploads-seed` and the entrypoint runs `rsync -a --ignore-existing /opt/uploads-seed/ /var/www/html/storage/app/public/` (busybox `cp -n` silently skipped new directories — do not replace rsync with cp).
8. **Deploys: `./scripts/deploy.sh`** — one command that does everything: git push → VPS `git pull && docker compose up -d --build` → **full local DB transfer** (`pg_dump` → drop schema → `pg_restore`) → storage rsync → `migrate` + cache warm + app restart → health check. **Rule: the local database is the source of truth — deploys replace the production database with the local one** (`--skip-db` to ship code only; production-only data such as new visitor comments is lost unless it exists locally). Admin credentials come from the local DB after a DB deploy. Seeding article seeders is rarely needed now (articles arrive with the DB) — use `./scripts/deploy.sh --skip-db SomeSeeder` for one-off data operations. Watch with `docker compose logs -f app caddy horizon`; `docker compose ps caddy` shows `127.0.0.1:8080->80`
